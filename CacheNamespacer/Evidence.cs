...
namespace CacheNamespacer
{
    public class Evidence
    {
        ...
        public Evidence(uint seed, int size)
        {
            ...
            prime = primes.Last(p => p < bits.Length);
        ...
        {
            bits[value % (bits.Length)] = true;
            bits[(value + offset) % (int)prime] = true;
        }

        public bool For(int value)
        {
            if (bits[value % (bits.Length)]==true
                && bits[(value + offset) % (int)prime] == true
                ) { return true; }
            return false;
        }

        static uint[] primes = new uint[] { 29,  71 , 113 , 173 , 229 , 281 , 349 , 409 , 463 , 541 , 601 , 659 , 733 , 809 , 863 ,
